...
边界也要说清楚：这是 IP/CIDR 系统级分流，不是域名级分流。Google 会动态调整 IP，维护者需要定期重新生成规则并提交，用户通过 `warp-vps update` 获取新快照。`warp-vps test/status` 只证明本机链路和规则命中正常，不等同于保证 Gemini、YouTube Premium 或具体地区策略一定解锁。
